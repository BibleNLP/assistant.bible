...
def test_chat_websocket_connection(mocker, fresh_db):
    ...
    # mocker.patch("app.routers.ConversationPipeline.llm_framework.generate_text",
    #                                                     return_value={"question": "Mock Question",
    #                                                         "answer": "Mock answer",
    #                                                         "source_documents": [],
    #                                                         })
...
def test_chat_based_on_translationwords_with_llm(mocker, fresh_db):
    ...
    with client.websocket_connect("/chat", params=args) as websocket:
        ...
        websocket.send_bytes(json.dumps({"message":"Can you tell me about angels?"}).encode('utf-8'))
        data = websocket.receive_json()
        assert_positive_bot_response(data)
        ...
        # websocket.send_bytes(json.dumps({"message":"How was earth in the beginning?"}).encode('utf-8'))
        # data = websocket.receive_json()
        # assert_positive_bot_response(data)
